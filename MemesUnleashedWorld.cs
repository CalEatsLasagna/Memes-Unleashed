...
{
	public class MemesUnleashedWorld : ModWorld
	{
		...
		public override void LoadLegacy(BinaryReader reader)
		{
			...
			if (loadVersion == 0)
			{
				...
				downedGiantGrasshopper = flags[0];
				downedBladeDemon = flags[1];
			}
			else
			{
		...
		public override void NetSend(BinaryWriter writer)
		{
			...
			flags[0] = downedGiantGrasshopper;
			flags[1] = downedBladeDemon;
			writer.Write(flags);
			...
			*/
			...
			//BitsByte flags = new BitsByte(downedGiantGrasshopper, downedBladeDemon);
			//writer.Write(flags);
		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
		{
			...
			if (ShiniesIndex != -1)
			{
				...
				tasks.Insert(ShiniesIndex + 1, new PassLegacy("MemesUnleashed Ores", MemesUnleashedOres));
			}
			int genIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Micro Biomes"));
			tasks.Insert(genIndex + 1, new PassLegacy("Shale Blob", delegate (GenerationProgress progress)
			{
			WorldGen.TileRunner(Main.spawnTileX - 450, Main.spawnTileY + -90, 70, Main.rand.Next(1, 3), mod.TileType("Shale"), true, 0f, 0f, true, true);
			}));
		}
		...
		private void MemesUnleashedOres(GenerationProgress progress)
		{
			...
			progress.Message = "Generating Static";
			...
			for (int k = 0; k < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 6E-04); k++)
			{
				...
				WorldGen.TileRunner(x, y, (double)WorldGen.genRand.Next(4, 6), WorldGen.genRand.Next(5, 6), mod.TileType("StaticOre"), false, 0f, 0f, false, true);
			}
			
			progress.Message = "Getting Hot and Spicy";
			...
			for (int k = 0; k < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 6E-04); k++)
			{
				...
				WorldGen.TileRunner(x, y, (double)WorldGen.genRand.Next(4, 6), WorldGen.genRand.Next(5, 6), mod.TileType("MagmiumOre"), false, 0f, 0f, false, true);
			}
		}
		...
		public override void PostWorldGen()
		{
			...
			int[] itemsToPlaceInIceChests = new int[] { mod.ItemType("MemeCoin"), mod.ItemType("SpaghettiFork"), ItemID.PinkJellyfishJar };
