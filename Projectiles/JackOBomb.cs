	...
	class JackOBomb : ModProjectile
	{
		...
		public override bool OnTileCollide(Vector2 oldVelocity)
		{
			...
			if (projectile.soundDelay == 0)
			{
				...
				Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/Bounce").WithVolume(.7f).WithPitchVariance(.5f));
			}
		...
		public override void AI()
		{
			...
			else
			{
				...
				if (Main.rand.Next(2) == 0)
				{
					...
					Main.dust[dustIndex].scale = 1.1f + (float)Main.rand.Next(5) * 2.3f;
			...
			if (projectile.ai[0] > 5f)
			{
				...
				{
					projectile.velocity.X = projectile.velocity.X * 0.77f;
					//if (projectile.type == 29 || projectile.type == 470 || projectile.type == 637)
					{
						projectile.velocity.X = projectile.velocity.X * 0.56f;
					}
